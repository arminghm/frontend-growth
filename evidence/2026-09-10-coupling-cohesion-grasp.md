...
Design Principles & Patterns block. The user analyzed frontend-specific responsibility-assignment and SOLID scenarios without hints: a profile form, product search, order history, article-draft persistence, and consumer-specific product data-source contracts.
...
- In the ISP exercise, extending the entire `ProductDetailsPage` contract from `AdminProductEditor` would be incorrect if that contract also contains `subscribeToStock`; the shared capability should be factored separately (for example `ProductReader { getById }`) or composed structurally so each consumer gets only what it needs.
...
- Continue applied SOLID with LSP, then OCP trade-offs.
